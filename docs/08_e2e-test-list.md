...
2. 「+ 新規ユーザー追加」をクリック
3. 名前: `新規管理者ユーザー` を入力
4. メールアドレス: `new-admin@example.com` を入力
5. ロール: `admin` を選択
6. 「作成」ボタンをクリック
...
**前提条件:**
- 管理者ユーザー（Admin）でログイン済み
- パスワードリセット対象の一般ユーザー（User A）が存在する

**テストステップ:**
1. ユーザー管理画面（`/admin/users`）にアクセス
2. User Aの「パスワードリセット」ボタンをクリック
3. 確認モーダルで「リセット」を確定

**期待結果:**
- 新しいパスワードがモーダルで表示される
- 成功メッセージ「パスワードをリセットしました」が表示される
- 新しいパスワードでUser Aがログインできる

---

#### E2E-ADMIN-008: 自分自身のパスワードリセット防止
...
#### E2E-DASHBOARD-001: ダッシュボードサマリーの表示
...
**テストステップ:**
1. ダッシュボード画面（`/admin/dashboard`）にアクセス
2. サマリーカードが表示されるまで待機
...
- 本日の会話数が表示される
- 総メッセージ数が表示される
- トークン使用量（入力/出力）が表示される
- 推定コスト（USD）が表示される

---

#### E2E-DASHBOARD-002: トレンドチャートの表示
...
#### E2E-DASHBOARD-003: ユーザーランキングの表示
...
**期待結果:**
- ユーザーランキングが表示される
- 各ユーザーのメールアドレス、名前、値が表示される
- メトリクス変更に応じてランキングが更新される

---

#### E2E-DASHBOARD-004: 一般ユーザーのダッシュボードアクセス拒否
...
**テストステップ:**
1. ダッシュボード画面（`/admin/dashboard`）に直接アクセス
...
#### E2E-DASHBOARD-005: カスタム期間でのトレンド表示
...
| 優先度 | テストケース数 | 説明 |
|-------|-------------|------|
| ★★★（クリティカル） | 15 | 必須実行、リリースブロッカー |
| ★★☆（重要） | 31 | 重要機能、プルリクエスト時に実行 |
| ★☆☆（通常） | 13 | 定期的に実行、リリース前に実行 |
...
frontend/
  e2e/
    tests/
      ...
      admin/
        user-management.spec.ts    # E2E-ADMIN-001 ~ E2E-ADMIN-008
        dashboard.spec.ts          # E2E-DASHBOARD-001 ~ E2E-DASHBOARD-005
      navigation/
