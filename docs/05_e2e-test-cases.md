...
### CHAT-005: 返信サジェスションの表示

- **画面**: /chat/:uuid
- **前提条件**: ログイン済み、AIからのレスポンスが完了している
- **操作手順**:
  1. メッセージを送信し、AIレスポンスの完了を待つ
- **期待結果**: メッセージ一覧の下部にサジェスションボタンが3〜5個表示される

### CHAT-006: サジェスションの選択による送信

- **画面**: /chat/:uuid
- **前提条件**: ログイン済み、サジェスションボタンが表示されている
- **操作手順**:
  1. 表示されたサジェスションボタンの1つをクリック
- **期待結果**: 選択したサジェスションがメッセージとして送信され、AIからの新しいレスポンスが表示される
