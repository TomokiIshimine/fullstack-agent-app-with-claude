...
**更新履歴:**
- v1.5 (2026-01-02): トラブルシューティングドキュメントを追加
- v1.4 (2025-12-27): ドキュメント内容を最新のアプリ構成に合わせて更新
...
graph TB
    ...
    DG --> TS["トラブルシューティング<br/>(10_troubleshooting.md)"]
    ...
    style TS fill:#f5f5f5,stroke:#333,stroke-width:2px
```
...
- E2Eテストシナリオ一覧（認証、チャット、ユーザー管理、セキュリティ等）
...
### 3.10 トラブルシューティング (10_troubleshooting.md)

**目的:**
- 開発環境やDocker構成で発生しやすい問題の解決手順を提供する
- 共通のエラーや手順を集約し、調査コストを下げる

**対象読者:**
- 新規参画者（セットアップ時の詰まり解消）
- 開発者（環境トラブル時の一次対応）
- DevOps担当（環境確認のチェックリスト）

**カバー範囲:**
- Docker コンテナやポート競合の切り分け
- MySQL/Redis の初期化・ヘルスチェック
- フロントエンド/バックエンドの起動エラー
- ログ確認とリセット手順

**他ドキュメントとの関係:**
- 開発環境ガイドの補足として参照
- テスト戦略書の前提となる環境整備情報を補完
...
10. **トラブルシューティング** (10_troubleshooting.md) - 環境トラブル時の参照先
...
|------------|--------------|--------------|
| **環境構築エラー** | トラブルシューティング、開発環境ガイド | Docker/ポート、初期セットアップ |
| **データベース接続エラー** | トラブルシューティング、開発環境ガイド、データベース設計書 | MySQL/Redis、データベース概要 |
| **認証エラー** | トラブルシューティング、認証・認可設計書 | 認証フロー、エラーパターン |
| **API エラー** | トラブルシューティング、API設計ガイド、機能一覧 | エラーハンドリング、APIエンドポイント一覧 |
| **テストエラー** | トラブルシューティング、テスト戦略書 | テスト実行環境、よくある失敗 |
