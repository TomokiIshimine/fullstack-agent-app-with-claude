...
def auth_service(app):
    ...
    Yields a tuple of (AuthService, session) so tests can commit
    the session when they need to verify changes in a new session.
    """
...
def test_login_success(app, test_user, auth_service):
    ...
    response_data, access_token, refresh_token = service.login("test@example.com", "password123")
...
def test_login_stores_refresh_token_in_database(app, test_user, auth_service):
    ...
    service, session = auth_service
    _, _, refresh_token = service.login("test@example.com", "password123")

    # Use the same session to verify (within same transaction)
...
def test_login_generates_valid_jwt_access_token(app, test_user, auth_service):
    ...
    service, session = auth_service
    _, access_token, _ = service.login("test@example.com", "password123")
...
def test_refresh_access_token_success(app, test_user, auth_service):
    ...
    # Refresh the token
    new_access_token, new_refresh_token, user = service.refresh_access_token(refresh_token)
...
def test_refresh_access_token_revokes_old_token(app, test_user, auth_service):
    ...
    _, new_refresh_token, _ = service.refresh_access_token(refresh_token)
...
def test_refresh_access_token_invalid_jwt(auth_service):
    ...
    with pytest.raises(InvalidRefreshTokenError):
        service.refresh_access_token("invalid_jwt_token")
...
def test_refresh_access_token_revoked(app, test_user, auth_service):
    ...
    # Manually revoke the token using the same session
    repo = RefreshTokenRepository(session)
...
def test_refresh_access_token_user_not_found(app, auth_service):
    ...
    # Store token in database using the same session
    repo = RefreshTokenRepository(session)
...
def test_logout_success(app, test_user, auth_service):
    ...
    # Use the same session to verify
    repo = RefreshTokenRepository(session)
    token_record = repo.find_by_token(refresh_token)
...
def test_generate_access_token_contains_user_info(app, test_user, auth_service):
    ...
    service, session = auth_service
    token = service._generate_access_token(test_user, "test@example.com", "user")
...
def test_generate_refresh_token_contains_jti(app, test_user, auth_service):
    ...
    service, session = auth_service
    token1 = service._generate_refresh_token(test_user)
    token2 = service._generate_refresh_token(test_user)
...
def test_full_token_lifecycle(app, test_user, auth_service):
    ...
    service, session = auth_service
    # Step 1: Login
    response_data, access_token_1, refresh_token_1 = service.login("test@example.com", "password123")
    assert response_data.user.id == test_user
    assert response_data.user.role == "user"
    assert response_data.user.name == "Test User"

    # Step 2: Refresh token
    access_token_2, refresh_token_2, user = service.refresh_access_token(refresh_token_1)
    assert user.id == test_user
    assert user.role == "user"
    assert user.name == "Test User"
    assert refresh_token_2 != refresh_token_1

    # Use the same session to verify old token is revoked, new token is active
    repo = RefreshTokenRepository(session)
    ...
    # Step 3: Logout
    service.logout(refresh_token_2)
...
def test_multiple_concurrent_sessions(app, test_user, auth_service):
    ...
    _, _, token1 = service.login("test@example.com", "password123")
    _, _, token2 = service.login("test@example.com", "password123")
    _, _, token3 = service.login("test@example.com", "password123")

    # Use the same session to verify all tokens are active
    repo = RefreshTokenRepository(session)
...
def test_token_refresh_prevents_replay_attack(app, test_user, auth_service):
    ...
    _, _, _ = service.refresh_access_token(new_refresh_token)
