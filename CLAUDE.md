...
Web application with AI chat functionality. Users can have real-time conversations with an AI assistant powered by Claude API.

### Features

| Feature | Description |
|---------|-------------|
| **AI Chat** | Real-time streaming conversations with Claude API, conversation history management |
| **Authentication** | JWT auth (httpOnly cookies), login/logout, token refresh |
| **User Management** | Admin user creation/deletion, profile editing, password change |
| **Role-Based Access** | Separation between admin and regular user permissions |

### Routes

- `/login` - Login page
- `/chat`, `/chat/:uuid` - AI chat page (main feature)
- `/settings` - User settings (profile, password change)
